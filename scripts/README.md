...
* **`generate_atlas_maps.py`**
  * **用途**：自 GGG 官方 PoE 2 CDN 下載並解析最新 `EndgameMaps.dat64` 與 `WorldAreas.dat64`，生成地圖名稱、標籤與頭目關聯資料。
...
* **`generate_atlas_content.py`**
  * **用途**：自 GGG 官方 PoE 2 CDN 下載並解析最新 `EndgameMapContent.dat64`，生成地圖機制內容、效果描述、圖示與中英對照。
  * **產出檔案**：預設為 `output/atlas_content.json`。
  * **執行方式**：
    ```bash
...
存放於 **`research/`** 目錄下，供反向工程與欄位分析使用：

* **`research/parse_endgame_maps.py`**
  * 基礎範例腳本，用於示範連線 CDN、讀取並印出單筆地圖的原始關聯資料。
* **`research/dump_raw_endgame_maps_zh.py`**
